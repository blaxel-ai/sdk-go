...
type ImageSummarySpec struct {
	...
	LastTagCreatedAt string `json:"lastTagCreatedAt"`
	Size             int64  `json:"size"`
	TagCount         int64  `json:"tagCount"`
	// At most three most recently created tags.
	TagsPreview []ImageTag `json:"tagsPreview"`
	// JSON contains metadata for fields, check presence with [respjson.Field.Valid].
	JSON struct {
		DefaultTag       respjson.Field
		LastTagCreatedAt respjson.Field
		Size             respjson.Field
		TagCount         respjson.Field
		TagsPreview      respjson.Field
		ExtraFields      map[string]respjson.Field
		raw              string
	} `json:"-"`
}
