...
func (r *ImageSharePendingService) Accept(ctx context.Context, pendingShareID string, opts ...option.RequestOption) (res *Image, err error) {
	opts = slices.Concat(r.Options, opts)
	if pendingShareID == "" {
		err = errors.New("missing required pendingShareId parameter")
		return nil, err
	}
	path := fmt.Sprintf("image-shares/pending/%s/accept", pendingShareID)
...
func (r *ImageSharePendingService) Decline(ctx context.Context, pendingShareID string, opts ...option.RequestOption) (err error) {
	...
	path := fmt.Sprintf("image-shares/pending/%s/decline", pendingShareID)
