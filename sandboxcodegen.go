...
func (r *SandboxCodegenService) Fastapply(ctx context.Context, path string, body SandboxCodegenFastapplyParams, opts ...option.RequestOption) (res *ApplyEditResponse, err error) {
	opts = slices.Concat(r.Options, opts)
	if path == "" {
		err = errors.New("missing required path parameter")
		return
	}
	path := fmt.Sprintf("codegen/fastapply/%s", path)
...
func (r *SandboxCodegenService) Reranking(ctx context.Context, path string, query SandboxCodegenRerankingParams, opts ...option.RequestOption) (res *RerankingResponse, err error) {
	opts = slices.Concat(r.Options, opts)
	if path == "" {
		err = errors.New("missing required path parameter")
		return
	}
	path := fmt.Sprintf("codegen/reranking/%s", path)
